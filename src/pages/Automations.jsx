...
const Automations = () => {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      (err) => {
        console.warn("campaigns listener error", err?.code || err);
        setCampaigns([]);
      }
    );
    return () => unsub();
  ...
  const saveCampaign = async (e) => {
    ...
    if (editing) {
      await updateDoc(
        doc(db, "users", user.uid, "campaigns", String(editing.id)),
  ...
  const removeCampaign = async (id) => {
    if (!user?.uid) return;
    await deleteDoc(doc(db, "users", user.uid, "campaigns", String(id)));
  ...
  const toggleStatus = async (c) => {
    ...
    if (!user?.uid) return;
    await updateDoc(doc(db, "users", user.uid, "campaigns", String(c.id)), {
      status: next,
    });
